...
failIfTxStatusError(sendContribution1_4Tx, sendContribution1Message + " - Blacklisted contribution ac6 400 ETH");
...
// The following block can only be execute if the threshold has not been reached
if (false) {
// -----------------------------------------------------------------------------
var reclaimEth1Message = "Reclaim ETH #1";
// -----------------------------------------------------------------------------
console.log("RESULT: ---------- " + reclaimEth1Message + " ----------");
var reclaimEth1_1Tx = crowdsale.reclaimEth({from: account8, gas: 1000000, gasPrice: defaultGasPrice});
while (txpool.status.pending > 0) {
}
printBalances();
